...
def _calculate_matches(
    ...
    gt_boxes: tf.Tensor,
    positive_iou_thresh: float,
    negative_iou_thresh: float,
    enforce_bipartite: bool = True,
):
    # Apply rules
...
def match_priors(
    ...
    gt_labels: tf.Tensor,
    gt_valid_mask: tf.Tensor | None,
    positive_iou_thresh: float,
    negative_iou_thresh: float,
    max_pos_per_gt: list[int] | None = None,
    allow_low_qual_matches: bool = True,
    ...
    match_dict = _calculate_matches(
        iou_matrix, valid_gt_boxes, positive_iou_thresh, negative_iou_thresh, enforce_bipartite=allow_low_qual_matches
    )
    ...
    matched_gt_idx = tf.where(
        match_dict["pos_mask"], match_dict["assigned_gt_box_index"], -tf.ones_like(match_dict["assigned_gt_box_index"])
    )
...
def hard_negative_mining(
    ...
    selected_negative_mask = tf.scatter_nd(
        indices=hard_negative_indices, updates=tf.ones(k, dtype=tf.bool), shape=[tf.shape(conf_loss)[0]]
    )
