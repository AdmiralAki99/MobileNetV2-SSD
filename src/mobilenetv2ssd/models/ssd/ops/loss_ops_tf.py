...
def multibox_loss(
    ...
    beta: float | None,
    cls_loss_type: str = "softmax_ce",
    loc_loss_type: str = "smooth_l1",
    normalize_denom: str = "num_pos",
    reduction: str = "sum",
    ...
    tf.boolean_mask(predicted_offsets_flattened, negative_mask_flattened)
    positive_targets_flattened = tf.boolean_mask(target_offsets_flattened, positive_mask_flattened)

    # Masking the logits
    tf.boolean_mask(predicted_logits_flattened, positive_mask_flattened)
    tf.boolean_mask(predicted_logits_flattened, negative_mask_flattened)
    ...
    if cls_loss_type == "softmax_ce":
        classification_raw = softmax_cross_entropy_loss(
            selected_prediction_logits, selected_prediction_targets, reduction=reduction
        )
    ...
    if loc_loss_type == "smooth_l1":
        if beta is not None:
            ...
            localization_raw = smooth_l1_loss(
                positive_offsets_flattened, positive_targets_flattened, beta=beta, reduction=reduction
            )
        else:
            if should_force_fp32("loss_reduction", precision_config):
                positive_offsets_flattened = tf.cast(positive_offsets_flattened, tf.float32)
            ...
            localization_raw = smooth_l1_loss(
                positive_offsets_flattened, positive_targets_flattened, beta=1.0, reduction=reduction
            )
