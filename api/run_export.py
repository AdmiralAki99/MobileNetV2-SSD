...
def parse_args():
    ...
    parser.add_argument(
        "--output_dir", required=False, default="exported_model", help="Output directory to store the exported model"
    )
...
def main():
    ...
    if not index_files:
        raise FileNotFoundError(f"No checkpoint files found in {local_checkpoint}." f"Check S3_PATH is correct.")
