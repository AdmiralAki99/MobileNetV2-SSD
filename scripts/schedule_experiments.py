...
def list_experiments_table(ledger: ExperimentLedger):
    ...
    for item in items:
        ...
        print(
            f"{item.get('experiment_id',''):<10} {item.get('fingerprint',''):<14} {item.get('status',''):<10} {str(item.get('priority','-')):<10} {str(item.get('total_steps','-')):<8} {metric_str:<10} {item.get('ec2_instance','-')}"
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--config_root", type=str, default=None, help="Path to configs root. Defaults to <project_root>/configs."
    )
    parser.add_argument("--table_name", type=str, default="ml-experiment-ledger", help="DynamoDB table name.")
    parser.add_argument("--region", type=str, default="us-east-1", help="AWS region.")
    parser.add_argument(
        "--git_commit", type=str, default=None, help="Git commit hash (must match what train.py will use)."
    )
    ...
    parser.add_argument(
        ...
        help="Reset all failed runs for an experiment_id to pending.",
    )
