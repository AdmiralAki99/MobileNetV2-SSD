...
class CheckpointManager:
    def __init__(
        ...
        self._best_metric_var = (
            tf.Variable(float("-inf"), dtype=tf.float32, trainable=False)
            if self._mode == "max"
            else tf.Variable(float("inf"), dtype=tf.float32, trainable=False)
        )
        ...
        checkpoint_dict["optimizer"] = self._optimizer
        ...
        self._last_manager = tf.train.CheckpointManager(
            checkpoint=self._checkpoint, directory=str(self._last_directory), max_to_keep=self._keep_last_k
        )
        ...
        if self._save_best:
            ...
            self._best_manager = tf.train.CheckpointManager(
                checkpoint=self._checkpoint, directory=str(self._best_directory), max_to_keep=1
            )
    ...
    def restore_latest(self):
        ...
        if latest_path is None:
            return {
                ...
                "epoch": 0,
                "global_step": 0,
                "best_metric": float("-inf") if self._mode == "max" else float("inf"),
                "best_epoch": -1,
            }
    ...
    def _normalize_optimizers(
        self, optimizer: tf.keras.optimizers.Optimizer | dict[str, tf.keras.optimizers.Optimizer]
    ):
...
def _create_checkpoint_directory_fingerprint(
    config: dict[str, Any], fingerprint: Fingerprint = None, job_dir: Path | str = None
):
...
def build_checkpoint_manager(
    config: dict[str, Any],
    model: tf.keras.Model,
    optimizer: tf.keras.optimizers.Optimizer | dict[str, tf.keras.optimizers.Optimizer],
    ema: Optional[Any],
    is_main_node: bool = True,
    ...
    checkpoint_config = _create_checkpoint_directory_fingerprint(
        config, fingerprint=None if fingerprint is None else fingerprint, job_dir=log_dir
    )

    checkpoint_manager = CheckpointManager(
        checkpoint_config, model, optimizer=optimizer, ema=ema, is_main_node=is_main_node
    )
