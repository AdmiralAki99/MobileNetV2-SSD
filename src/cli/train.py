...
def parse_args():
    ...
    parser.add_argument(
        "--config_root", type=str, default=PROJECT_ROOT / "configs", help="Root directory for configuration files."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Specific checkpoint step to resume from. Defaults to the latest.",
    )
    ...
    if args["resume"]:
        ...
        if runs:
            ...
            if selected is not None:
                ...
                compatibility_flag, warnings = validate_checkpoint_compatibility(
                    saved_config=saved_config, current_config=config
                )
...
def create_datasets(config: dict[str, Any], logger: Logger):
    ...
    logger.info(
        f"Training transforms: {[transform.__class__.__name__ for transform in train_compose._transforms]} {'.'*20}"
    )
    logger.info(
        f"Validation transforms: {[transform.__class__.__name__ for transform in validation_compose._transforms]} {'.'*20}"
    )
    ...
    if not use_tfrecords:
        ...
        logger.info(
            f"Created {training_dataset.__class__.__name__} training dataset with {len(training_dataset)} samples {'.'*20}"
        )
        logger.info(
            f"Train loop has {int(len(training_dataset) / config['data']['train']['batch_size'])} steps{'.'*20}"
        )

        if config["eval"]["eval_enabled"]:
            validation_dataset = create_dataset_from_config(config=config, split=config["data"]["val_split"])
            logger.info(
                f"Created {validation_dataset.__class__.__name__} validation dataset with {len(validation_dataset)} samples {'.'*20}"
            )
            logger.info(
                f"Eval loop has {int(len(validation_dataset) / config['data']['val']['batch_size'])} steps{'.'*20}"
            )
        ...
        val_dataset = (
            create_validation_dataset(config=config, dataset=validation_dataset, transform=validation_compose)
            if config["eval"]["eval_enabled"]
            else None
        )
        ...
        train_dataset = create_training_dataset_from_tfrecords(
            config=config, shard_paths=train_shard_paths, transform=train_compose
        )
        ...
        logger.info(
            f"TFRecord training dataset: {len(train_shard_paths)} shards, {n_train} samples, ~{n_train // config['data']['train']['batch_size']} steps/epoch {'.'*20}"
        )
        ...
        if config["eval"]["eval_enabled"]:
            ...
            val_dataset = create_validation_dataset_from_tfrecords(
                config=config, shard_paths=val_shard_paths, transform=validation_compose
            )

            n_val = shard_metadata[config["data"]["val_split"]]
            logger.info(
                f"TFRecord validation dataset: {len(val_shard_paths)} shards, {n_val} samples, ~{n_val // config['data']['val']['batch_size']} steps/epoch {'.'*20}"
            )
        else:
            val_dataset = None
...
def create_ema(config: dict[str, Any], model: tf.keras.Model, logger: Logger):
    ...
    logger.info(
        f"EMA name: {ema.name}, decay: {ema._decay}, enabled: {ema._enabled}, warmup steps: {ema._warmup_steps}, update every: {ema._update_every}, eval_use_ema: {ema._eval_use_ema}"
    )

    return ema


def create_build_checkpoint_manager(
    config: dict[str, Any],
...
def initialize_framework(args: dict[str, Any]):
    ...
    logger.success(
        f"Successfully Initialized Forced FP32 for {precision_config.get_forced_precision_fields()}...{'.'*20}"
    )

    logger.info(f"Wrapping Optimizer for AMP...{'.'*20}")
    optimizer = amp.wrap_optimizer()
    logger.info(
        f"Optimizer class variable: {optimizer.__class__.__name__} with inner Optimizer: {optimizer.inner_optimizer.__class__.__name__ if isinstance(optimizer, tf.keras.mixed_precision.LossScaleOptimizer) else 'NA'}"
    )
    ...
    checkpoint_manager = create_build_checkpoint_manager(
        config=config, model=model, logger=logger, ema=ema, optimizer=optimizer, fingerprint=fingerprint
    )
    logger.success("Successfully Created Checkpoint Manager")
...
def train(framework_opts: TrainingBundle, shutdown_handler: ShutdownHandler, resume_ckpt_path: Path | None = None):
    ...
    framework_opts.logger.save_model_weights(
        framework_opts.model, training_result, framework_opts.config, framework_opts.fingerprint, framework_opts.ema
    )
