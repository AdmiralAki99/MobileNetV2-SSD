...
class Logger:
    ...
    def _log(self, level: str, message: str, **extra):
        ...
        record = self._logger.makeRecord(
            name=self._logger.name, level=level_config.level, fn="", lno=0, msg=message, args=(), exc_info=None
        )
    ...
    def log_metrics(
        ...
        self._metric_history.append({"step": step, "timestamp": datetime.now().isoformat(), **prefixed})

    def log_training_step(
        self, step: int, loss: float, learning_rate: float, extra: dict[str, float] | None = None, log_every: int = 100
    ):
    ...
    def start_tensorboard(self, port: int = 6006, host: str = "0.0.0.0", log_dir: str | Path | None = None):
        ...
        if is_port_in_use(port):
            ...
            self.info("To use a different port, call: logger.start_tensorboard(port=XXXX)")
            return None
        ...
        # Start TensorBoard
        cmd = ["tensorboard", "--logdir", str(target_dir), "--port", str(port), "--host", host]

        self.info(f"Starting TensorBoard on port {port}...")
        self.info(f"Log directory: {target_dir}")

        self._tensorboard_process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    def save_model_weights(
        ...
        if ema is not None and ema.enabled and ema.eval_use_ema:
            ...
            self.checkpoint("Applied EMA shadow weights to model.")
