...
from tensorflow.keras.layers import Conv2D, BatchNormalization, ReLU, DepthwiseConv2D
...
class StandardConvolutionBlock(tf.keras.layers.Layer):
    def __init__(self, output_channels=32, stride=2, alpha=1.0, name=None):
        ...
        output_channel = max(
            1, int(output_channels * alpha)
        )  # To scale the proportions to make them fit for different devices
...
class InvertedResidualBlock(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        if self.expansion_factor != 1:
            ...
            self.expansion_conv = Conv2D(
                ...
                use_bias=False,
                kernel_initializer="he_normal",
                name=None if self.name is None else f"{self.name}_expand_conv",
            )
            self.expand_batch_norm = BatchNormalization(name=None if self.name is None else f"{self.name}_expand_bn")
            self.expand_activation_function = ReLU(
                max_value=6.0, name=None if self.name is None else f"{self.name}_expand_relu6"
            )
        ...
        self.depthwise_conv = DepthwiseConv2D(
            ...
            name=None if self.name is None else f"{self.name}_dw_conv",
        )
        self.depthwise_batch_norm = BatchNormalization(name=None if self.name is None else f"{self.name}_dw_bn")
        self.depthwise_activation_function = ReLU(
            max_value=6.0, name=None if self.name is None else f"{self.name}_dw_relu6"
        )
        ...
        self.projection_conv = Conv2D(
            ...
            use_bias=False,
            kernel_initializer="he_normal",
            name=None if self.name is None else f"{self.name}_project_conv",
        )
    ...
    def transplant_weights(self, reference_table: dict, name_dict: dict = None):
        ...
        if self.expansion_factor == 1:
            self._assign_conv_weights(
                self.depthwise_conv, self._find_ref(reference_table, name_dict, f"{self.name}_dw_conv")
            )
            self._assign_bn_weights(
                self.depthwise_batch_norm, self._find_ref(reference_table, name_dict, f"{self.name}_dw_bn")
            )
            self._assign_conv_weights(
                self.projection_conv, self._find_ref(reference_table, name_dict, f"{self.name}_project_conv")
            )
            self._assign_bn_weights(
                self.project_batch_norm, self._find_ref(reference_table, name_dict, f"{self.name}_project_bn")
            )
        else:
            self._assign_conv_weights(
                self.expansion_conv, self._find_ref(reference_table, name_dict, f"{self.name}_expand_conv")
            )
            self._assign_bn_weights(
                self.expand_batch_norm, self._find_ref(reference_table, name_dict, f"{self.name}_expand_bn")
            )
            self._assign_conv_weights(
                self.depthwise_conv, self._find_ref(reference_table, name_dict, f"{self.name}_dw_conv")
            )
