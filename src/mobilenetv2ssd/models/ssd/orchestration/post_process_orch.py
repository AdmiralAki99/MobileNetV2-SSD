...
def _read_eval_config(config: dict[str, Any]):
    ...
    eval_config = {
        ...
        "variances": tf.constant(
            list(config.get("eval", {}).get("decode", {}).get("variances", [0.1, 0.2])), dtype=tf.float32
        ),
        "use_sigmoid": config.get("eval", {}).get("decode", {}).get("use_sigmoid", False),
        "input_size": {
            "image_height": config.get("eval", {}).get("input", config["input_size"])[0],
            "image_width": config.get("eval", {}).get("input", config["input_size"])[1],
        },
    }
...
def build_decoded_boxes(
    ...
    nmsed_boxes, nmsed_scores, nmsed_classes, valid_detections = decode_and_nms(
        ...
        priors=priors,
        variances=eval_config["variances"],
        scores_thresh=eval_config["score_threshold"],
        iou_thresh=eval_config["iou_threshold"],
        top_k=eval_config["per_class_top_k"],
        max_detections=eval_config["max_detections_per_image"],
        image_meta=eval_config["input_size"],
        use_sigmoid=eval_config["use_sigmoid"],
