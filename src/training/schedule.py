...
class CosineWarmupSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __call__(self, step: tf.Tensor):
        ...
        cosine_decay = self.minimum_learning_rate + 0.5 * (self.base_learning_rate - self.minimum_learning_rate) * (
...
class LearningRateSchedulerFactory:
    ...
    def _build_learning_schedule(config: dict[str, Any]):
        ...
        if config["learning_schedule"] == "cosine_warmup":
            learning_schedule = CosineWarmupSchedule(
                base_learning_rate=config["base_learning_rate"],
                minimum_learning_rate=config["mininum_learning_rate"],
                warmup_steps=config["warmup_steps"],
                total_steps=config["total_steps"],
            )
        elif config["learning_schedule"] == "step_decay":
            learning_schedule = StepDecaySchedule(
                base_learning_rate=config["base_learning_rate"],
                gamma=config["step_gamma"],
                milestones=config["multistep_milestones"],
            )
        elif config["learning_schedule"] == "exponential_decay":
            learning_schedule = ExponentialDecaySchedule(
                base_learning_rate=config["base_learning_rate"],
                gamma=config["step_gamma"],
                decay_interval=config["step_drop_every_epochs"],
            )
