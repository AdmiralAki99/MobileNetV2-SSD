...
class ConsensusEngine:
    ...
    def compute(self, detections_per_model: dict[str, list[Detection]]):
        flattened_list = [(model_name, detection) for model_name, detection_list in detections_per_model.items() for detection in detection_list]
        ...
        for root, indices in clusters.items():
            ...
            majority_class = max(set(detection.class_id for detection in cluster_detections), key=lambda c: sum(1 for detection in cluster_detections if detection.class_id == c))
            class_name = next(detection.class_name for detection in cluster_detections if detection.class_id == majority_class)
            model_confidences = {flattened_list[i][0]: flattened_list[i][1].confidence for i in indices}
            
            consensus_annotations.append(ConsensusAnnotation(
                box=avg_box,
