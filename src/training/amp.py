...
class AMPContext:
    ...
    def setup_policy(self):
        ...
        if self._policy not in {"mixed_float16", "mixed_bfloat16", "float32"}:
            raise ValueError(
                f"AMP policy name is not valid, error value: {self._policy}, allowed values: {['mixed_float16', 'mixed_bfloat16', 'float32']}"
            )
    ...
    def wrap_optimizer(self):
        ...
        if self._loss_scale == "dynamic":
            ...
            self.optimizer = tf.keras.mixed_precision.LossScaleOptimizer(
                self._base_optimizer, initial_scale=loss_scale_mode
            )
