...
class ClassificationHead(tf.keras.Layer):
    def __init__(
        ...
        norm_cfg: str = "BatchNorm",
        head_type: str = "conv3x3",
    ...
    def call(self, feature_maps, training=False):
        ...
        for layer, feature_map in enumerate(feature_maps):
            ...
            tf.debugging.assert_equal(
                tf.shape(x)[-1],
                tf.constant(num_anchors * C, dtype=tf.int32),
                message=f"Expected last dimension to be {num_anchors * C} but got {tf.shape(x)[-1]}",
            )
            ...
            tf.debugging.assert_equal(
                tf.size(x), expected, message="Total element count mismatch before flatten reshape"
            )
