...
def encode_boxes_batch(
    matched_gt_xyxy: tf.Tensor,
    priors_cxcywh: tf.Tensor,
    variances: tuple[float, float],
    precision_config: PrecisionConfig | None = None,
):
    # Need to create a function that encodes boxes by batch
    tf.shape(matched_gt_xyxy)[0]

    # Map over the entire batch
    batched_offsets = tf.map_fn(
        lambda matched_boxes: encode_boxes_core(matched_boxes, priors_cxcywh, variances, precision_config),
        elems=matched_gt_xyxy,
