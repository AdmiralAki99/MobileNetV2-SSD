...
def _get_pending_videos(config_path: str) -> list:
    ...
    if bucket and not LOCAL_MODE:
        ...
        if not os.path.exists(video_dir):
            return []
        all_files = [os.path.join(video_dir, f) for f in os.listdir(video_dir) if f.lower().endswith(extensions)]
...
def _run_etl_job(**context):
    ...
    if LOCAL_MODE:
        ...
        result = subprocess.run(
            [
                ...
                "--rm",
                "--network",
                "mobilenetv2-ssd_default",
                "-v",
                f"{host_dir}/configs:/app/configs",
                "-v",
                f"{host_dir}/videos:/app/videos",
                "-v",
                f"{host_dir}/datasets:/app/datasets",
                "mobilenetv2-ssd-etl:latest",
                "--config",
                "/app/configs/etl/default.yaml",
                "--videos",
                *video_paths,
            ],
            capture_output=True,
    ...
    resp = requests.post(
        ...
        json={"entrypoint": f"python -m src.cli.etl --config {config_path} --videos {videos_arg}"},
    )
...
def _send_summary_email(**context):
    ...
    class_rows_html = "".join(f"<tr><td>{r[0]}</td><td>{r[1]}</td></tr>" for r in class_rows)
...
with DAG(
    ...
    provision_ec2 = BashOperator(
        ...
        bash_command=(
            'echo "local mode - skipping EC2 provisioning"'
            if LOCAL_MODE
    ...
    teardown_ec2 = BashOperator(
        ...
        bash_command=(
            'echo "local mode - skipping EC2 teardown"'
            if LOCAL_MODE
