...
from mobilenetv2ssd.core.utils import (
    ssd_get_prior_stats,
    calculate_model_prediction_health,
    calculate_nms_health_scores,
    calculate_gt_health_scores,
    calculate_pred_health_metrics,
    verify_pred_boxes_sanity,
    gt_box_range,
    calculate_iou_sanity_top1,
    prediction_box_bad_frac,
...
def training_step(
    ...
    tf.debugging.assert_equal(
        tf.rank(image), tf.constant(4, dtype=tf.int32), message=f"The image has rank : {tf.rank(image)}, expected: 4"
    )

    # Building the training targets
    localization_targets, classification_targets, positive_mask, negative_mask, ignore_mask, diagnostics = (
        building_training_targets(
            config=config,
    ...
    tf.debugging.assert_equal(
        tf.shape(classification_targets)[:2],
        tf.shape(localization_targets)[:2],
        message=f"The shapes between the are different between classification targets:{tf.shape(classification_targets)[:2]}, but expected {tf.shape(localization_targets)[:2]}",
    )

    predicted_offsets, predicted_logits = model(image, training=True)

    tf.debugging.assert_equal(
        tf.shape(predicted_logits)[:2],
        tf.shape(localization_targets)[:2],
        message=f"The shapes between the are different between localization logits :{tf.shape(predicted_logits)[:2]}, but expected {tf.shape(localization_targets)[:2]}",
    )

    tf.debugging.assert_equal(
        tf.shape(localization_targets),
        tf.shape(predicted_offsets),
        message=f"The shapes between the are different between localization targets:{tf.shape(localization_targets)}, and predicted_offsets:{tf.shape(predicted_offsets)}",
    )

    # Calculating the confidence loss
    conf_loss, candidate_negative_mask = build_conf_loss(
        config=config,
    ...
    selected_negative_mask = select_hard_negatives(
        config=config, conf_loss=conf_loss, positive_mask=positive_mask, negative_mask=candidate_negative_mask
    )
    ...
    # Calculate the loss
    loss_dict = calculate_final_loss(
        config=config,
        ...
        negative_mask=selected_negative_mask,
        precision_config=precision_config,
    )
...
def train_one_epoch(
    ...
    def _compiled_step(batch: dict[str, Any]):
        with tf.GradientTape() as tape:
            # Using the AMP
            with amp.autocast():
                loss_dict, prior_stats = training_step(
                    config=config,
                    model=model,
                    priors_cxcywh=priors_cxcywh,
                    batch=batch,
        ...
        gradients = tape.gradient(
            scaled_loss, model.trainable_variables, unconnected_gradients=tf.UnconnectedGradients.ZERO
        )

        scaled_grads = [
            grad * backbone_grad_scale if is_backbone else grad for grad, is_backbone in zip(gradients, backbone_mask)
        ]
    ...
    for step, batch in enumerate(train_dataset):
        ...
        if step % log_every == 0:
            logger.metric(
                f"Epoch {epoch}, Step {step}, Loss {float(total_loss.numpy())}, Num Pos: {int(loss_dict['num_pos'].numpy())}"
            )
            ...
            if isinstance(optimizer, tf.keras.mixed_precision.LossScaleOptimizer):
                logger.log_scalar(
                    tag="train/lr", value=optimizer.inner_optimizer.learning_rate.numpy(), step=global_step
                )
            else:
...
def evaluate_step(
    ...
    # Postprocess
    nmsed_boxes, nmsed_scores, nmsed_classes, decoded_classes, classes, valid_detections = build_decoded_boxes(
        config=config,
        predicted_offsets=predicted_offsets,
        predicted_logits=predicted_logits,
        priors=priors_cxcywh,
        precision_config=precision_config,
    )
...
def evaluate(
    ...
    def _compiled_eval_step(batch):
        return evaluate_step(
            config=config, model=model, priors_cxcywh=priors_cxcywh, batch=batch, precision_config=precision_config
        )
    ...
    with ema.eval_context(model):
        for step, batch in enumerate(val_dataset):
            ...
            # Compute the metrics
            predictions, ground_truths = convert_batch_images_to_metric_format(
                pred_boxes=evaluation_output["pred_boxes"],
                pred_scores=evaluation_output["pred_scores"],
                pred_labels=evaluation_output["pred_classes"],
                gt_boxes=evaluation_output["gt_boxes"],
                gt_labels=evaluation_output["gt_labels"],
                gt_mask=evaluation_output["gt_mask"],
            ...
            if step % heavy_log_every == 0:
                ...
                pred_health_metrics = calculate_pred_health_metrics(
                    evaluation_output["pred_scores"],
                    evaluation_output["pred_classes"],
                    evaluation_output["valid_detections"],
                )
                pred_box_sanity = verify_pred_boxes_sanity(
                    evaluation_output["pred_boxes"], evaluation_output["valid_detections"]
                )
                prediction_health = calculate_model_prediction_health(
                    evaluation_output["predicted_logits"], evaluation_output["predicted_offsets"]
                )
                ...
                logger.metric(
                    f"Average Ground Truth Count Per Image: {ground_truth_health_metrics['avg_ground_truth_boxes_per_image']}"
                )
                ...
                logger.metric(
                    f"Mean background probability: {tf.reduce_mean(prediction_health['background_probs']).numpy()}"
                )
                logger.metric(
                    f"Max background probability: {tf.reduce_max(prediction_health['background_probs']).numpy()}"
                )

                logger.metric(
                    f"Mean top foreground probability: {tf.reduce_mean(tf.reduce_max(prediction_health['foreground_probs'], axis=-1)).numpy()}"
                )
                logger.metric(
                    f"Mean sum foreground probability: {tf.reduce_mean(tf.reduce_sum(prediction_health['foreground_probs'], axis=-1)).numpy()}"
                )
                logger.metric(
                    f"Max foreground probability: {tf.reduce_max(tf.reduce_sum(prediction_health['foreground_probs'], axis=-1)).numpy()}"
                )

                logger.metric(f"Predicted Logits mean: {tf.reduce_mean(prediction_health['predicted_logits']).numpy()}")
                logger.metric(
                    f"Predicted Logits std: {tf.math.reduce_std(prediction_health['predicted_logits']).numpy()}"
                )
            ...
            if (step + 1) % log_every == 0:
                ...
                gt_box_sanity = gt_box_range(evaluation_output["gt_boxes"], evaluation_output["gt_mask"])
                iou_sanity = calculate_iou_sanity_top1(
                    evaluation_output["pred_boxes"],
                    evaluation_output["pred_scores"],
                    evaluation_output["valid_detections"],
                    evaluation_output["gt_boxes"],
                    evaluation_output["gt_mask"],
                )
                pred_bad_boxes_ratio = prediction_box_bad_frac(
                    evaluation_output["pred_boxes"], evaluation_output["valid_detections"]
                )
                ground_truth_bad_box_ratio = ground_truth_box_bad_frac(
                    evaluation_output["gt_boxes"], evaluation_output["gt_mask"]
                )
                ...
                logger.log_scalar(
                    tag="val/nms_num_valid_scores", value=nms_health_metrics["num_valid"], step=eval_step_offset + step
                )
                logger.log_scalar(
                    tag="val/nms_min_valid_scores", value=nms_health_metrics["min_valid"], step=eval_step_offset + step
                )
                logger.log_scalar(
                    tag="val/nms_mean_valid_scores",
                    value=nms_health_metrics["mean_valid"],
                    step=eval_step_offset + step,
                )
                logger.log_scalar(
                    tag="val/nms_max_valid_scores", value=nms_health_metrics["max_valid"], step=eval_step_offset + step
                )
                logger.log_scalar(
                    tag="val/nms_num_valid_scores_less_than_0.9",
                    value=nms_health_metrics["below_thresh_scores"],
                    step=eval_step_offset + step,
                )
                logger.log_scalar(
                    tag="val/nms_average_valid_detections",
                    value=nms_health_metrics["average_valid_det"],
                    step=eval_step_offset + step,
                )
                logger.log_scalar(
                    tag="val/nms_zero_valid_detections_ratio",
                    value=nms_health_metrics["zero_valid_det"],
                    step=eval_step_offset + step,
                )
                logger.log_scalar(
                    tag="val/nms_mean_top1_scores", value=nms_health_metrics["mean_top1"], step=eval_step_offset + step
                )
                logger.log_histogram(
                    tag="val/nms_top1_scores_incl0_detections",
                    values=nms_health_metrics["top1_incl0"],
                    step=eval_step_offset + step,
                )
                logger.log_scalar(
                    tag="val/nms_mean_top1_scores_incl0_detections",
                    value=nms_health_metrics["mean_top1_incl0"],
                    step=eval_step_offset + step,
                )
                ...
                logger.metric(
                    f"Zero Ground Truth Ratio Per Batch: {ground_truth_health_metrics['zero_ground_truth_ratio']}"
                )

                logger.metric(f"Ground Truth Boxes Bad Box Ratio : {ground_truth_bad_box_ratio}")
                logger.log_histogram(
                    tag="val/gt_top_class_dist",
                    values=ground_truth_health_metrics["top_gt_class_distribution"],
                    step=eval_step_offset + step,
                )

                logger.log_histogram(
                    tag="val/gt_count_per_image",
                    values=ground_truth_health_metrics["ground_truth_count"],
                    step=eval_step_offset + step,
                )
                logger.log_scalar(
                    tag="val/gt_avg_count_per_image",
                    value=ground_truth_health_metrics["avg_ground_truth_boxes_per_image"],
                    step=eval_step_offset + step,
                )
                logger.log_scalar(
                    tag="val/gt_zero_ratio_per_batch",
                    value=ground_truth_health_metrics["zero_ground_truth_ratio"],
                    step=eval_step_offset + step,
                )
                ...
                logger.log_histogram(
                    tag="val/pred_top_class_dist",
                    values=pred_health_metrics["top_class_distribution"],
                    step=eval_step_offset + step,
                )
                logger.log_scalar(
                    tag="val/pred_boxes_bad_ratio", value=pred_bad_boxes_ratio, step=eval_step_offset + step
                )
...
def fit(
        ...
        for epoch in range(start_epoch, epochs):
            ...
            if epoch % eval_every == 0 or epoch == epochs - 1:
                ...
                eval_metrics, global_eval_step = evaluate(
                    ...
                    eval_step_offset=global_eval_step,
    ...
    return {"best_metric": best_metric, "primary_metric": primary_metric, "global_step": global_step}
