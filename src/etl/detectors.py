    ...
    def predict(self, image: np.ndarray) -> list[Detection]:
        ...
        for box in result.boxes:
            ...
            if coco_cls in COCO_TO_VISDRONE:
                ...
                detections.append(
                    Detection(box=np_box, class_id=visdrone_cls, class_name=visdrone_label, confidence=conf)
                )
...
class GroundingDINODetector(BaseDetector):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.model_id = (
            config.get("models", {})
            .get("grounding_dino_model", {})
            .get("model_id", "IDEA-Research/grounding-dino-tiny")
        )
    ...
    def predict(self, image: np.ndarray):
        ...
        for box, label, score in zip(result["boxes"], result[label_key], result["scores"]):
            ...
            detections.append(
                Detection(
                    box=norm_box, class_id=visdrone_id, class_name=VISDRONE_CLASSES[visdrone_id], confidence=score
                )
            )
