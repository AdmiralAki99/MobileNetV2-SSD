...
class PhotometricDistort:
    ...
    def distort_image(self, image):
        image = self.determine_outcome(
            lambda x: tf.image.random_brightness(x, self._brightness_delta, seed=self._seed), image
        )
    ...
    def contrast_first(self, image):
        image = self.determine_outcome(
            lambda x: tf.image.random_contrast(x, self._contrast_range[0], self._contrast_range[1], seed=self._seed),
            image,
        )
        image = self.determine_outcome(
            lambda x: tf.image.random_saturation(
                x, self._saturation_range[0], self._saturation_range[1], seed=self._seed
            ),
            image,
        )
...
class RandomHorizontalFlip:
    ...
    def __call__(self, image, target):
        ...
        image, target = tf.cond(
            random_num < self._p, lambda: self.horizontal_flip(image, target), lambda: self.dont_flip(image, target)
        )
...
def build_train_augmentation_config(config: dict[str, Any]):
    ...
    augment_config = {
        ...
        "pipeline": augment_opts.get(
            "pipeline", ["photometric_distort", "random_flip", "resize", "sanitize_boxes", "normalize"]
        ),
        "params": {
            "random_flip": {
...
def build_validation_augmentation_config(config: dict[str, Any]):
    ...
    validation_config = {
        ...
        "standardize_pipeline": validation_opts.get(
            "standardize_pipeline", ["to_float32", "sanitize_boxes", "scale_01"]
        ),
