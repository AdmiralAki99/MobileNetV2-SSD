...
from datasets.base import DetectionSample, create_dataset_from_config
...
def encode_sample(sample: DetectionSample):
    ...
    example = encode_features(
        boxes=encoded_boxes,
        boxes_count=encoded_boxes_count,
        labels=encoded_labels,
        height=encoded_height,
        width=encoded_width,
        image_id=encoded_image_id,
...
def create_tfrecords_from_dataset(config: dict[str, Any], shard_size=500):
    ...
    print(
        f"Training Dataset of {training_dataset.__class__.__name__}, with split: { config['data']['train_split']}..{'.'*20}"
    )
    write_split(
        dataset=training_dataset, output_dir=output_dir, split_name=training_dataset.split, shard_size=shard_size
    )
    ...
    print(
        f"Validation Dataset of {val_dataset.__class__.__name__}, with split: { config['data']['val_split']}..{'.'*20}"
    )
    ...
    metadata = {
        config["data"]["train_split"]: len(training_dataset),
        config["data"]["val_split"]: len(val_dataset),
    }
...
if __name__ == "__main__":
    ...
    config = load_config("../configs/experiments/exp003_visdrone_run.yaml")
