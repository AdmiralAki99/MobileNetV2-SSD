...
def create_validation_dataset(config: dict[str, Any], dataset: BaseDetectionDataset, transform: Compose):
    ...
    tf_dataset = tf_dataset.padded_batch(
        ...
        padded_shapes=dataset_opts["padded_shapes"],
        padding_values={
...
def create_training_dataset_from_tfrecords(config: dict[str, Any], shard_paths: list[str] | None, transform: Compose):
    ...
    tf_dataset = tf.data.TFRecordDataset(filenames=shard_paths, num_parallel_reads=tf.data.AUTOTUNE).map(
        _parse_tfrecord, num_parallel_calls=tf.data.AUTOTUNE
    )
    ...
    tf_dataset = tf_dataset.prefetch(
        tf.data.AUTOTUNE
    )  # High speed rails so there is no need to stop this if sharding is done
