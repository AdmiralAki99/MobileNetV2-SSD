...
def calculate_model_prediction_health(predicted_logits: tf.Tensor, predicted_offsets: tf.Tensor):
    ...
    tf.nn.softmax(predicted_logits, axis=-1)  # correct (classes)
    tf.nn.softmax(predicted_logits, axis=1)  # wrong (priors)
...
def calculate_nms_health_scores(pred_scores: tf.Tensor, valid_detections: tf.Tensor):
    ...
    # Min valid score
    min_valid = tf.cond(
        num_valid > 0, lambda: tf.reduce_min(valid_scores), lambda: tf.constant(0.0, dtype=pred_scores.dtype)
    )

    # Mean valid score
    mean_valid = tf.cond(
        num_valid > 0, lambda: tf.reduce_mean(valid_scores), lambda: tf.constant(0.0, dtype=pred_scores.dtype)
    )

    # Max valid score
    max_valid = tf.cond(
        num_valid > 0, lambda: tf.reduce_max(valid_scores), lambda: tf.constant(0.0, dtype=pred_scores.dtype)
    )
    ...
    # Mean Top 1 scores
    mean_top1 = tf.cond(
        tf.size(valid_top1) > 0, lambda: tf.reduce_mean(valid_top1), lambda: tf.constant(0.0, dtype=pred_scores.dtype)
    )
...
def calculate_gt_health_scores(
    ground_truth_boxes: tf.Tensor, ground_truth_labels: tf.Tensor, ground_truth_mask: tf.Tensor, select_k: int = 10
):
...
def calculate_pred_health_metrics(
    ...
    if classes_per_batch.shape.rank == 0:
        return {"top_classes": [], "top_class_countes": [], "top_class_distribution": tf.constant([], dtype=tf.int32)}
...
def verify_pred_boxes_sanity(pred_boxes: tf.Tensor, valid_detections):
    ...
    return {"min_coordinates": min_coordinate.numpy().tolist(), "max_coordinates": max_coordinate.numpy().tolist()}
...
def calculate_iou_sanity_top1(
    pred_boxes: tf.Tensor,
    pred_scores: tf.Tensor,
    valid_detections: tf.Tensor,
    ground_truth_boxes: tf.Tensor,
    gt_mask: tf.Tensor,
    include_no_detection_as_zero: bool = True,
):
    ...
    mean_only_detections = (
        tf.reduce_mean(tf.stack(ious_only_det)) if len(ious_only_det) > 0 else tf.constant(0.0, tf.float32)
    )
...
def draw_bounding_boxes(
    ...
    image_tensor: tf.Tensor | None = None,
):
    ...
    def label_color(label):
        return ((37 * label + 17) % 256, (57 * label + 101) % 256, (83 * label + 59) % 256)
...
def inference_function(
    ...
    if hasattr(class_labels, "numpy"):
        ...
        class_labels = {
            k: (
                v.numpy().decode("utf-8")
                if hasattr(v, "numpy") and isinstance(v.numpy(), bytes)
                else str(v.numpy()) if hasattr(v, "numpy") else v
            )
    ...
    img = draw_bounding_boxes(
        ...
        image_id=image_id,
        boxes=valid_gt,
        labels=valid_gt_labels,
        pred_boxes=top_k_boxes,
        pred_scores=top_k_scores,
        pred_labels=top_k_labels,
        dataset_name=config["data"]["dataset_name"],
        dataset_root=config["data"]["root"],
        labels_map=class_labels,
        image_tensor=image,
    )
