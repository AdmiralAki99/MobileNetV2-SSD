...
def test_batched_zero_area_box_xyxy_to_cxcywh():
    box_xyxy_batched = tf.constant(
        [[[1.0, 1.0, 1.0, 1.0], [2.0, 2.0, 3.0, 3.0]], [[4.0, 4.0, 4.0, 4.0], [5.0, 5.0, 6.0, 6.0]]], dtype=tf.float32
    )

    expected_boxes_cxcywh_batched = tf.constant(
        [[[1.0, 1.0, 0.0, 0.0], [2.5, 2.5, 1.0, 1.0]], [[4.0, 4.0, 0.0, 0.0], [5.5, 5.5, 1.0, 1.0]]], dtype=tf.float32
    )
...
def test_clip_xyxy_batched():
    box_xyxy_batched = tf.constant(
        [
            [[-10.0, 20.0, 130.0, 250.0], [50.0, -30.0, 220.0, 180.0]],
            [[15.0, 25.0, 350.0, 450.0], [-50.0, 65.0, 75.0, 85.0]],
        ],
        dtype=tf.float32,
    )

    expected_clipped_box_batched = tf.constant(
        [[[0.0, 20.0, 130.0, 100.0], [50.0, 0.0, 200.0, 100.0]], [[15.0, 25.0, 200.0, 100.0], [0.0, 65.0, 75.0, 85.0]]],
        dtype=tf.float32,
    )
...
def test_area_xyxy_core_simple():
    boxes_xyxy = tf.constant([[0.0, 0.0, 10.0, 10.0], [5.0, 5.0, 15.0, 20.0], [2.0, 3.0, 8.0, 9.0]], dtype=tf.float32)
...
def test_intersection_xyxy_core_simple():
    boxes1 = tf.constant([[0.0, 0.0, 10.0, 10.0], [5.0, 5.0, 15.0, 20.0]], dtype=tf.float32)

    boxes2 = tf.constant([[5.0, 5.0, 15.0, 15.0], [0.0, 0.0, 10.0, 10.0]], dtype=tf.float32)
