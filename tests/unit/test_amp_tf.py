...
def test_check_mixed_precision():
    ...
    assert isinstance(
        amp.optimizer, tf.keras.mixed_precision.LossScaleOptimizer
    ), f"AMP wrapped assert failed expected {tf.keras.optimizers.loss_scale_optimizer.LossScaleOptimizer} for {type(amp.optimizer)} "

    assert y_pred.dtype == tf.float16, f"AMP predicition dtype mismatch, expected: {tf.float16}, got {y_pred.dtype}"

    assert (
        model.trainable_variables[0].dtype == tf.float32
    ), f"AMP predicition dtype mismatch, expected: {tf.float32}, got {model.trainable_variables[0].dtype}"
