...
def parse_args():
    ...
    parser.add_argument(
        "--camera", type=str, default="0", help="Camera source: device index (default 0) or HTTP stream URL."
    )
    parser.add_argument(
...
def draw_detections(
    img: Image.Image, boxes_norm: np.ndarray, scores: np.ndarray, class_ids: np.ndarray, labels: list[str]
) -> Image.Image:
...
def draw_detections_cv2(
    frame: np.ndarray, boxes_norm: np.ndarray, scores: np.ndarray, class_ids: np.ndarray, labels: list[str]
) -> np.ndarray:
    import cv2
...
def execute_inference() -> int:
    try:
        ...
        for image_path in image_paths:
            ...
            det_summary = [
                (labels[c] if c < len(labels) else str(c), f"{s:.2f}") for c, s in zip(final_classes, final_scores)
            ]
