...
from mobilenetv2ssd.models.ssd.ops.encode_ops_tf import encode_boxes_batch
...
def building_training_targets(
    config: dict[str, Any],
    priors_cxcywh: tf.Tensor,
    gt_boxes_xyxy: tf.Tensor,
    gt_labels: tf.Tensor,
    gt_valid_mask: tf.Tensor,
    ...
    matched_dict = tf.map_fn(
        lambda inputs: match_priors(
            priors_cxcywh=priors_cxcywh,
            gt_boxes_xyxy=inputs[0],
            gt_labels=inputs[1],
            gt_valid_mask=inputs[2],
            positive_iou_thresh=target_config["iou_threshold_pos"],
            negative_iou_thresh=target_config["iou_threshold_neg"],
            max_pos_per_gt=None,
            allow_low_qual_matches=target_config["allow_low_quality_matches"],
            center_in_gt=target_config["center_in_gt"],
            return_iou=target_config["diagnostics"],
    ...
    localization_targets = encode_boxes_batch(
        matched_gt_xyxy=matched_dict["matched_gt_xyxy"],
        priors_cxcywh=priors_cxcywh,
        variances=tuple(target_config["variances"]),
        precision_config=precision_config,
    )
