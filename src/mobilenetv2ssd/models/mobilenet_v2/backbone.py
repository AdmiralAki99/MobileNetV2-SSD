...
class MobileNetV2(tf.keras.Model):
    def __init__(self, name="backbone", alpha=1.0, **kwargs):
        ...
        self.bottleneck_1 = InvertedResidualBlock(
            output_channels=16, stride=1, expansion_factor=1, alpha=alpha, name="bottleneck_block_1"
        )
        self.bottleneck_2 = InvertedResidualBlock(
            output_channels=24, stride=2, expansion_factor=6, alpha=alpha, name="bottleneck_block_2"
        )
        self.bottleneck_3 = InvertedResidualBlock(
            output_channels=24, stride=1, expansion_factor=6, alpha=alpha, name="bottleneck_block_3"
        )
        self.bottleneck_4 = InvertedResidualBlock(
            output_channels=32, stride=2, expansion_factor=6, alpha=alpha, name="bottleneck_block_4"
        )
        self.bottleneck_5 = InvertedResidualBlock(
            output_channels=32, stride=1, expansion_factor=6, alpha=alpha, name="bottleneck_block_5"
        )
        self.bottleneck_6 = InvertedResidualBlock(
            output_channels=32, stride=1, expansion_factor=6, alpha=alpha, name="bottleneck_block_6"
        )
        self.bottleneck_7 = InvertedResidualBlock(
            output_channels=64, stride=2, expansion_factor=6, alpha=alpha, name="bottleneck_block_7"
        )
        self.bottleneck_8 = InvertedResidualBlock(
            output_channels=64, stride=1, expansion_factor=6, alpha=alpha, name="bottleneck_block_8"
        )
        self.bottleneck_9 = InvertedResidualBlock(
            output_channels=64, stride=1, expansion_factor=6, alpha=alpha, name="bottleneck_block_9"
        )
        self.bottleneck_10 = InvertedResidualBlock(
            output_channels=64, stride=1, expansion_factor=6, alpha=alpha, name="bottleneck_block_10"
        )
        self.bottleneck_11 = InvertedResidualBlock(
            output_channels=96, stride=1, expansion_factor=6, alpha=alpha, name="bottleneck_block_11"
        )
        self.bottleneck_12 = InvertedResidualBlock(
            output_channels=96, stride=1, expansion_factor=6, alpha=alpha, name="bottleneck_block_12"
        )
        self.bottleneck_13 = InvertedResidualBlock(
            output_channels=96, stride=1, expansion_factor=6, alpha=alpha, name="bottleneck_block_13"
        )
        self.bottleneck_14 = InvertedResidualBlock(
            output_channels=160, stride=2, expansion_factor=6, alpha=alpha, name="bottleneck_block_14"
        )
        self.bottleneck_15 = InvertedResidualBlock(
            output_channels=160, stride=1, expansion_factor=6, alpha=alpha, name="bottleneck_block_15"
        )
        self.bottleneck_16 = InvertedResidualBlock(
            output_channels=160, stride=1, expansion_factor=6, alpha=alpha, name="bottleneck_block_16"
        )
        self.bottleneck_17 = InvertedResidualBlock(
            output_channels=320, stride=1, expansion_factor=6, alpha=alpha, name="bottleneck_block_17"
        )
        self.conv_2 = Conv2D(
            self._make_divisible(int(round(1280 * alpha))) if alpha >= 1 else 1280,
            kernel_size=1,
            strides=1,
            padding="same",
            use_bias=False,
    ...
    def _assign_conv_weights(self, destination, source):
        ...
        assert len(destination_weights) == len(
            source_weights
        ), f"{destination.name}: variable count mismatch {len(destination_weights)} != {len(source_weights)}"
    ...
    def _find_ref(self, reference_table, name_dict, lookup_name):
        ...
        if isinstance(lookup_layer, str):
...
def build_backbone(
    ...
    weights_dir: str | None = _DEFAULT_WEIGHTS_DIR,
):
    ...
    ref_model = tf.keras.applications.MobileNetV2(
...
standard_conv_block_name_map = {"Conv1_conv": "Conv1", "Conv1_bn": "bn_Conv1", "Conv1_relu6": "Conv1_relu"}

final_block_name_map = {"conv_head": "Conv_1", "conv_head_bn": "Conv_1_bn", "conv_head_relu": "out_relu"}
bottleneck_block_1 = {
