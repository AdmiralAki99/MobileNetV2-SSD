...
def select_hard_negatives(
    config: dict[str, Any], conf_loss: tf.Tensor, positive_mask: tf.Tensor, negative_mask: tf.Tensor
):
    ...
    selected_negative_mask = tf.map_fn(
        lambda inputs: hard_negative_mining(
            conf_loss=inputs[0],
            pos_mask=inputs[1],
            neg_mask=inputs[2],
            neg_ratio=sampler_config["neg_pos_ratio"],
            min_neg=sampler_config["min_neg"],
            max_neg=sampler_config["max_neg"],
        ),
