...
def compute_scales_per_layer(
    scales: list[float] | None, number_of_layers: int, s_min: float, s_max: float, include_extra: bool = False
):
    ...
    if scales is not None:
        ...
        for scale in scales:
            assert 0.0 < scale <= 1.0, "Scales must be in the in space (0,1]"
    else:
        assert 0.0 < s_min <= s_max <= 1.0, "Require 0 < s_min ≤ s_max ≤ 1"
        scales = [
            _calculate_scale_at_layer_number(k, s_min, s_max, number_of_layers) for k in range(1, number_of_layers + 1)
        ]

    if include_extra:
        return [
            (
                [scales[k], _calculate_default_box_scale(scales[k], 1)]
                if k == number_of_layers - 1
                else [scales[k], _calculate_default_box_scale(scales[k], scales[k + 1])]
            )
...
def compute_meta(
    layers_priors: list[tf.Tensor],
    image_size: tuple[int, int],
    strides: list[float] | None,
    feature_map_shapes: list[tuple[int, int]],
    scales_per_layer: list[[float]],
    aspect_ratios_per_layer: list[[float]],
):
    # Validate inputs
    length = len(feature_map_shapes)
    assert (
        length == len(layers_priors) == len(scales_per_layer) == len(aspect_ratios_per_layer)
    ), "Metadata lists must all have the same length (one entry per layer)."
    ...
    tf.debugging.assert_equal(
        cell_sanity_check,
        tf.ones_like(cell_sanity_check, dtype=tf.bool),
        "No. of anchors per layer needs to be divisible by height * width in each layer",
    )
    ...
    tf.concat([tf.zeros_like(layer_sizes[:1]), layer_sizes[:-1]], axis=0)
    ...
    meta["fingerprint"] = _build_fingerprint(
        {
...
def build_priors(
    ...
    feature_map_shapes: list[tuple[int, int]] | None = None,
    scales: list[list[float]] | list[float] | None = None,
    aspect_ratios: list[list[float]] | list[float] | None = None,
    ...
    prior_layers = [
        build_layer_priors(feature_map_shapes[i], image_size, scales[i], ratios[i], dtype, clip)
