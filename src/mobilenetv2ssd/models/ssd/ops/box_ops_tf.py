...
@tf.function(
    input_signature=[tf.TensorSpec(shape=[None, 4], dtype=tf.float32), tf.TensorSpec(shape=[None, 4], dtype=tf.float32)]
)
...
def resize_xyxy_core(
    boxes,
    source_height: tf.float32,
    source_width: tf.float32,
    destination_height: tf.float32,
    destination_width: tf.float32,
):
...
def resize_xyxy_batched(
    ...
    flattened_boxes = tf.reshape(boxes, [-1, 4])
    flipped_boxes = resize_xyxy_core(
        flattened_boxes, source_height, source_width, destination_height, destination_width
    )
