...
def test_creating_translation_block():
    translation_block_2 = {
        "bottleneck_block_2_expand_conv": "block_1_expand",
        "bottleneck_block_2_expand_bn": "block_1_expand_BN",
        "bottleneck_block_2_expand_relu6": "block_1_expand_relu",
        "bottleneck_block_2_dw_conv": "block_1_depthwise",
        "bottleneck_block_2_dw_bn": "block_1_depthwise_BN",
        "bottleneck_block_2_dw_relu6": "block_1_depthwise_relu",
        "bottleneck_block_2_project_conv": "block_1_project",
        "bottleneck_block_2_project_bn": "block_1_project_BN",
    }
...
def test_smoke_forward_pass():
    ...
    for key in expected_keys:
        assert (
            output[key].shape[0] == tensor_shape[0]
        ), f"The batch dimension for {key} are wrong: expected {tensor_shape[0]}, got {output[key].shape[0]}"
...
def test_spatial_downscaling():
    ...
    for key in expected_downscaling.keys():
        assert (
            math.ceil(input_shape[0] / output[key].shape[1]) == expected_downscaling[key]
            and math.ceil(input_shape[1] / output[key].shape[2]) == expected_downscaling[key]
        ), f"The downsampling for {key} is wrong: expected {expected_downscaling[key]}, got {(math.ceil(input_shape[0]/output[key][1]), math.ceil(input_shape[1]/output[key][2]))}"
...
def test_model_scaling():
    ...
    for alpha in expected_channels.keys():
        assert (
            expected_channels[alpha] == observed_channels[alpha]
        ), f"Unexpected channel scaling for {alpha}: expected {expected_channels[alpha]}, got {observed_channels[alpha]}"
